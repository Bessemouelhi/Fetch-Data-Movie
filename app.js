...
async function fetchFilm(name) {
    try {
        ...
        results.innerHTML = Object.values(data.results).map((movie, index) => `
            <div class="cards">
                <img src="${movie.poster_path != null ? 'https://image.tmdb.org/t/p/w200'+movie.poster_path : 'img/sans-affiche.png'}" alt="">
                <div>
                    <h3><i>Titre</i> : ${movie.title}</h3>
                    <h5>V.O. : ${movie.original_language}</h5>
                    <p><i>Résumé</i> : ${movie.overview}</p>
                    <h4><i>Date de sortie</i> : ${movie.release_date}</h4>
                    <h4><i>Note</i> : ${movie.vote_average} / 10 </h4>
